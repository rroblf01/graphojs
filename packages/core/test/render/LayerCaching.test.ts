...
describe('Diagram layer caching', () => {
  it('should be disabled by default', () => {
    ...
    expect(diagram.isLayerCachingEnabled()).toBe(false);
    diagram.destroy();
  });
  ...
  it('should enable layer caching', () => {
    const div = document.createElement('div');
    const diagram = new Diagram({ div });
    diagram.enableLayerCaching(1);
    expect(diagram.isLayerCachingEnabled()).toBe(true);
    diagram.destroy();
  });

  it('should disable layer caching', () => {
    const div = document.createElement('div');
    const diagram = new Diagram({ div });
    diagram.enableLayerCaching(1);
    diagram.disableLayerCaching();
    expect(diagram.isLayerCachingEnabled()).toBe(false);
    diagram.destroy();
  });

  it('should render with layer caching enabled', () => {
    const div = document.createElement('div');
    const diagram = new Diagram({ div });
